...
int pthread_mutex_timedlock_fake(pthread_mutex_t **uid, const struct timespec *abs) {
  pthread_mutex_t *m; int ret = ensure_mutex(uid, &m);
  if (ret != 0) return ret;
  /* Round 161: honour the deadline instead of a hard-coded 1000 ms -- see
   * nx_abs_to_ms in libc_shim.c for why a spurious timeout is not harmless. */
  extern unsigned nx_abs_to_ms_pub(const struct timespec *abs, unsigned cap_ms);
  unsigned ms = nx_abs_to_ms_pub(abs, 30000u);
...
static uint64_t *condhop_for(pthread_cond_t *c) {
  ...
  for (unsigned i = 0; i < CONDHOP_N; i++) {
    ...
    if (g_condhop[k].c == 0) { g_condhop[k].c = c; g_condhop[k].hop = FN_FUTEX_HOP_MIN;
...
int pthread_cond_wait_fake(pthread_cond_t **cnd, pthread_mutex_t **mtx) {
  ...
  uint64_t hop = hopp ? *hopp : FN_FUTEX_HOP_MIN;
  struct timespec cap;
  ...
  if (hopp) {
    if (r == ETIMEDOUT) { uint64_t nh = hop << 2;
                          *hopp = (nh > FN_FUTEX_HOP_MAX) ? FN_FUTEX_HOP_MAX : nh; }
    else *hopp = FN_FUTEX_HOP_MIN;             /* real signal -> responsive again */
  }
  diag_wait_exit();
...
static EGLBoolean egl_SwapBuffers_log(EGLDisplay d, EGLSurface s) {
  ...
  if (g_swap_count <= FN_SWAP_FINISH_N) {
...
extern DynLibFunction fruitninja_extra_functions[];  /* imports_fruitninja_extra.c */
extern size_t fruitninja_extra_numfunctions;
...
static void build_combined(void) {
  ...
  g_combined_n = (int)dynlib_numfunctions + unity_dynlib_numfunctions
               + (int)firebase_extra_numfunctions + (int)fruitninja_extra_numfunctions;
  ...
  memcpy(g_combined + off, fruitninja_extra_functions,
         fruitninja_extra_numfunctions * sizeof(DynLibFunction));
}
