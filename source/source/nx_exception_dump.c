...
#include "util.h"
#include "so_util.h"
#include "nx_patch_fruitninja.h"   /* FN_IL2CPP_LIVENESS_* frame map (round 129) */
...
/* 0xDE is FN_POISON_FREE's fill (nx_alloc.c). A pointer built out of poison
 ...
 * from the caller's frame -- see the frame map in nx_patch_fruitninja.h. */
/* The ConvertFromTo fault. Frame map and its cross-check: nx_patch_fruitninja.h.
...
static void xd_databinding_triage(ThreadExceptionDump *ctx, u64 rel_pc) {
  ...
  if (rel_pc != (u64)FN_IL2CPP_GETTYPE_LEAF) return;
  if ((ctx->lr.x - (u64)g_il2cpp_base) != (u64)FN_IL2CPP_CFT_LR) {
  ...
  if (!xd_readable(sp + FN_CFT_SP_VALUE, 8)) { debugPrintf("[xd]   frame unreadable\n"); return; }
  { const u64 framed = *(const u64 *)(sp + FN_CFT_SP_VALUE);
  ...
  if (xd_readable(sp + FN_CFT_SP_DATABINDING, 8)) {
    const u64 db = *(const u64 *)(sp + FN_CFT_SP_DATABINDING);
    ...
    if (xd_readable((uintptr_t)db, 0x40)) {
      ...
      xd_managed_string("  m_propertyName",
                        *(const u64 *)(uintptr_t)(db + FN_DATABINDING_PROPNAME));
      xd_managed_string("  m_dataPath",
                        *(const u64 *)(uintptr_t)(db + FN_DATABINDING_DATAPATH));
      xd_managed_string("  m_stringFormat",
                        *(const u64 *)(uintptr_t)(db + FN_DATABINDING_STRFMT));
    }
  }
  ...
  { const u64 rt = ctx->cpu_gprs[20].x;
    if (xd_readable((uintptr_t)rt, 0x18)) {
      const u64 ty = *(const u64 *)(uintptr_t)(rt + FN_REFLECTIONTYPE_TYPE);
   ...
   * and dump what it was actually reading -- see nx_patch_fruitninja.h for the
  ...
  if (!xd_readable(sp + FN_SDS_SP_PIECE, 8)) return;
  { const u64 dlg   = *(const u64 *)(sp + FN_SDS_SP_DIALOGUE);
    const u64 piece = *(const u64 *)(sp + FN_SDS_SP_PIECE);
    ...
    if (xd_readable((uintptr_t)dlg, 0x40)) {
      const u64 cfg = *(const u64 *)(uintptr_t)(dlg + FN_DIALOGUE_CONFIG);
      const int  idx = *(const int *)(uintptr_t)(dlg + FN_DIALOGUE_INDEX);
      ...
      if (xd_readable((uintptr_t)cfg, 0x58)) {
        ...
        const u64 arr = *(const u64 *)(uintptr_t)(cfg + FN_DLGCONFIG_PIECES);
        debugPrintf("[xd]   m_dialoguePieces %s\n", xd_sym(arr, b, sizeof b));
        xd_klass_name("  pieces[]", arr);
        if (xd_readable((uintptr_t)arr, 0x40)) {
          const unsigned n = *(const unsigned *)(uintptr_t)(arr + FN_IL2CPP_ARRAY_LEN);
          debugPrintf("[xd]     length=%u\n", n);
          for (unsigned i = 0; i < n && i < 6; i++) {
            const uintptr_t ep = (uintptr_t)arr + FN_IL2CPP_ARRAY_DATA + i * 8u;
...
static void xd_liveness_triage(ThreadExceptionDump *ctx, u64 rel_pc) {
  if (rel_pc < FN_IL2CPP_LIVENESS_ADD_LO || rel_pc > FN_IL2CPP_LIVENESS_ADD_HI) return;
  extern uintptr_t g_il2cpp_base;
  const uintptr_t sp = (uintptr_t)ctx->sp.x;
  ...
  if (xd_readable((uintptr_t)obj, 8)) {
    ...
    if (xd_readable((uintptr_t)k, 0x140)) {
      const u64 hier = *(volatile const u64 *)(uintptr_t)(k + FN_IL2CPP_KLASS_TYPEHIER);
      const u8  dep  = *(volatile const u8  *)(uintptr_t)(k + FN_IL2CPP_KLASS_DEPTH);
    ...
    if (xd_readable((uintptr_t)state, 0x10)) {
      ...
      if (xd_readable((uintptr_t)filt, 0x140)) {
        ...
        debugPrintf("[xd]   walk filter   : %016lx %.60s depth=%u"
                    ...
                    *(volatile const u8 *)(uintptr_t)(filt + FN_IL2CPP_KLASS_DEPTH));
      } else {
  ...
  if (rel_lr == FN_IL2CPP_LIVENESS_LR_INST && xd_readable(sp + 0x40, 0x10)) {
  ...
  { extern uintptr_t g_il2cpp_base;
    ...
    if (ib && rel_pc < 0x370a000ull) {
      const char *what =
        ...
        (rel_pc == (u64)FN_IL2CPP_LIVENESS_HASPAR)
                                 ? "liveness HasParent klass->typeHierarchy[] "
                                   "(PAST the r119 guard -- guard took, value differed)" :
        (rel_pc == (u64)FN_IL2CPP_GETTYPE_LEAF) ? "Object.GetType leaf   (DataBinding/ConvertFromTo path)" :
